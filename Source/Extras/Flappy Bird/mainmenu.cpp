...
void mainmenu()
{
	...
	ALLEGRO_SAMPLE_INSTANCE* BGM_mainmenu_instance = al_create_sample_instance(BGM_mainmenu);
	al_set_sample_instance_playmode(BGM_mainmenu_instance, ALLEGRO_PLAYMODE_LOOP);
	al_set_sample_instance_gain(BGM_mainmenu_instance, regVolume(volBGM-0.8));
	al_attach_sample_instance_to_mixer(BGM_mainmenu_instance, al_get_default_mixer());
	al_play_sample_instance(BGM_mainmenu_instance);
	...
	while(!exit_program && ! exit_screen)
	{
        ...
        || ev.type == ALLEGRO_EVENT_JOYSTICK_AXIS )
		{
            ...
            {
               option = !option;
               al_play_sample(SFX_menusound1, regVolume(volSFX-0.4), 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
            }
            ...
            {
               al_play_sample(SFX_menusound2, regVolume(volSFX-0.4), 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
               if(option == 1)
                exit_program = true;
               else
               {
	...
	al_destroy_sample_instance(BGM_mainmenu_instance);
